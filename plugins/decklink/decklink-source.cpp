...
#ifdef _WIN32
	decklink->enable3d = obs_data_get_bool(settings, ENABLE_3D);
	decklink->rightEye = obs_data_get_int(settings, EYE_SELECT) == 1;
#endif
...
#ifdef _WIN32
	obs_data_set_default_bool(settings, ENABLE_3D, false);
	obs_data_set_default_int(settings, EYE_SELECT, 0);
#endif
}
...
#ifdef _WIN32
static bool enable_3d_changed(obs_properties_t *props, obs_property_t *, obs_data_t *settings)
{
	const bool enabled = obs_data_get_bool(settings, ENABLE_3D);
	obs_property_t *eye = obs_properties_get(props, EYE_SELECT);
	if (eye)
		obs_property_set_visible(eye, enabled);
	return true;
}
#endif
...
#ifdef _WIN32
	obs_property_t *enable3d = obs_properties_add_bool(props, ENABLE_3D, TEXT_ENABLE_3D);
	obs_property_set_modified_callback(enable3d, enable_3d_changed);

	obs_property_t *eye =
		obs_properties_add_list(props, EYE_SELECT, TEXT_EYE_SELECT, OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
	obs_property_list_add_int(eye, TEXT_EYE_LEFT, 0);
	obs_property_list_add_int(eye, TEXT_EYE_RIGHT, 1);
	bool showEye = false;
	if (data) {
		auto *decklink = static_cast<DeckLinkInput *>(data);
		obs_data_t *settings = obs_source_get_settings(decklink->GetSource());
		showEye = obs_data_get_bool(settings, ENABLE_3D);
		obs_data_release(settings);
	}
	obs_property_set_visible(eye, showEye);
#endif
