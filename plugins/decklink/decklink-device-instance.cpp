...
void DeckLinkDeviceInstance::HandleVideoFrame(IDeckLinkVideoInputFrame *videoFrame, const uint64_t timestamp)
{
	...
	IDeckLinkVideoFrame *inputFrame = videoFrame;
#ifdef _WIN32
	if (enable3d && rightEye) {
		ComPtr<IDeckLinkVideoFrame3DExtensions> frame3d;
		if (SUCCEEDED(videoFrame->QueryInterface(IID_IDeckLinkVideoFrame3DExtensions, (void **)&frame3d)) &&
		    frame3d) {
			ComPtr<IDeckLinkVideoFrame> rightFrame;
			if (SUCCEEDED(frame3d->GetFrameForRightEye(&rightFrame)) && rightFrame) {
				inputFrame = rightFrame.Get();
			}
		}
	}
#endif
	...
	if (inputFrame->GetPixelFormat() != convertFrame->GetPixelFormat()) {
		ComPtr<IDeckLinkVideoConversion> frameConverter;
		frameConverter.Set(CreateVideoConversionInstance());

		frameConverter->ConvertFrame(inputFrame, convertFrame);
	...
	if (frame->GetFlags() & bmdFrameContainsHDRMetadata) {
		...
		if (SUCCEEDED(frame->QueryInterface(IID_IDeckLinkVideoFrameMetadataExtensions, (void **)&metadata))) {
			int64_t range;
			if (SUCCEEDED(
...
#ifdef _WIN32
	enable3d = static_cast<DeckLinkInput *>(decklink)->enable3d;
	rightEye = static_cast<DeckLinkInput *>(decklink)->rightEye;
#endif
...
#ifdef _WIN32
	if (enable3d)
		flags = (BMDVideoInputFlags)(flags | bmdVideoInputDualStream3D);
#endif
